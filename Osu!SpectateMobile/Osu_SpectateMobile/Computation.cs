...
namespace OsuSpectate
{
    ...
    {
        public static float XComputation(Position p, float OriginX, float OriginY, float width, float height, int windowWidth, int windowHeight)
        {
            float xrelative;
            float w = width * windowWidth;
            float h = height * windowHeight;
            if (w / h > 640.0f / 480.0f)
            {
                xrelative = ((p.X) / (640.0f) * (h * 640.0f / 480.0f) + ((w - h * 640.0f / 480.0f) / (2.0f))) / w;
            }
            else
            {
                xrelative = (p.X) / (640.0f);
            }
            return width * xrelative + OriginX;
        }
        public static float YComputation(Position p, float OriginX, float OriginY, float width, float height, int windowWidth, int windowHeight)
        {
            float yrelative;
            float w = width * windowWidth;
            float h = height * windowHeight;
            if (w / h > 19.0f / 15.0f)
            {
                yrelative = (480.0f - p.Y) / (480.0f);
            }
            else
            {
                yrelative = ((480.0f - p.Y) / (480.0f) * (w * 480.0f / 640.0f) + ((h - w * 480.0f / 640.0f) / (2.0f))) / h;
            }
        ...
        public static float XComputation(float x, float OriginX, float OriginY, float width, float height, int windowWidth, int windowHeight)
        {
            ...
            if (w / h > 640.0f / 480.0f)
            {
                xrelative = ((x) / (640.0f) * (h * 640.0f / 480.0f) + ((w - h * 640.0f / 480.0f) / (2.0f))) / w;
            }
            else
            {
                xrelative = (x) / (640.0f);
        ...
        public static float YComputation(float y, float OriginX, float OriginY, float width, float height, int windowWidth, int windowHeight)
        {
            ...
            if (w / h > 640.0f / 480.0f)
            {
                yrelative = (480.0f-y) / (480.0f);
            }
            else
            {
                yrelative = ((480.0f - y) / (480.0f) * (w * 480.0f / 640.0f) + ((h - w * 480.0f / 640.0f) / (2.0f))) / h;
            }
            return height * yrelative + OriginY;
        }
